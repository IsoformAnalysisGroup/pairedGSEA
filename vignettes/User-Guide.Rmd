...
Install development version from [GitHub](https://github.com/IsoformAnalysisGroup/pairedGSEA)
...
By default, `paired_ora()` applies effect-size filtering before ORA:
expression genes must pass `abs(log2FC) >= log2(1.5)`, and splicing
genes must pass `abs(dIF) >= 0.1`. The splicing effect size is stored as
`max_abs_dif_splicing` by `paired_diff()` and is calculated from isoform
fractions using `IsoformSwitchAnalyzeR::isoformToIsoformFraction()`. To
use the previous adjusted p-value-only behavior, set
